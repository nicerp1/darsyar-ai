...
const Admin = (function () {
    ...
    function renderProgram(schedule) {
        ...
        (Array.isArray(schedule) ? schedule : []).forEach((day, dayIndex) => (day.slots || []).forEach((slot, slotIndex) => { if (slot.subject) slots.push({ ...slot, day: day.day, dayIndex, slotIndex }); }));
        box.innerHTML = slots.map(slot => `<div class="admin-program-item" style="--slot-color:${esc(slot.color || '#0ea5e9')}"><div><strong>${esc(slot.subject)}</strong><span>${esc(slot.day)} · ${esc(slot.time)}</span><small>${esc(slot.note || 'بدون توضیح')}</small></div><button class="admin-icon-button" aria-label="حذف ${esc(slot.subject)}" onclick="Admin.removeProgramSlot(${slot.dayIndex},${slot.slotIndex})"><span class="material-symbols-outlined" aria-hidden="true">delete</span></button></div>`).join('') || '<div class="admin-list-empty">هنوز برنامه‌ای ثبت نشده است.</div>';
    }
    ...
    async function removeProgramSlot(dayIndex, slotIndex) { if (!selectedUsername) return; try { const response = await fetch('/api/admin-students', { method: 'DELETE', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: selectedUsername, dayIndex, slotIndex }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error); selectedPayload.data.schedule = payload.schedule; renderProgram(payload.schedule); Utils.showToast('بازه برنامه حذف شد.', 'info'); } catch (error) { Utils.showToast(error.message, 'error'); } }
