...
const Admin = (function () {
    ...
    function renderStudentDetail() {
        ...
        renderProgram(data.schedule || []); renderReport(history, results, reports, Array.isArray(data.chat_reports) ? data.chat_reports : []);
    }
    ...
    function renderReport(history, results, reports, chatReports) {
        ...
        const safety = chatReports.slice(-5).reverse().map(item => `<li class="chat-report-item"><span>${esc(item.reason || 'پیام گزارش‌شده')}</span><small>${esc(item.snapshot?.text || '')}</small></li>`).join('');
        box.innerHTML = `<div class="counselor-report-table"><table><thead><tr><th>تاریخ</th><th>درس و توضیح</th><th>بازه</th><th>مدت</th><th>نتیجه</th></tr></thead><tbody>${rows || '<tr><td colspan="5">هنوز گزارش دستی ثبت نشده است.</td></tr>'}</tbody></table></div><aside><h5>گزارش‌های ایمنی گفت‌وگو</h5><ul>${safety || '<li><span>پیام گزارش‌شده‌ای وجود ندارد.</span></li>'}</ul><p>${pd(history.length)} روز سابقه مطالعه ثبت شده است.</p></aside>`;
    }
