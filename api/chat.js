...
module.exports = async (req, res) => {
    try {
        ...
        const username = actor.username === 'kiankaki' ? requested : actor.username;
        if (!username || username === 'kiankaki') return res.status(400).json({ error: 'دانش‌آموز معتبر انتخاب نشده است.' });
        if (actor.username === 'kiankaki' && !(await db(`profiles?username=eq.${encodeURIComponent(username)}&select=username`))?.length) return res.status(404).json({ error: 'دانش‌آموز پیدا نشد.' });
        ...
        messages.push({ id: `${Date.now()}-${crypto.randomBytes(4).toString('hex')}`, sender: actor.username === 'kiankaki' ? 'advisor' : 'student', senderName: actor.username === 'kiankaki' ? 'مشاور' : actor.name || actor.username, text, createdAt: new Date().toISOString() });
